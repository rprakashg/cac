...
import (
	...
	"github.com/rprakashg/cac/pkg/k8s/selector/fields"
	"github.com/rprakashg/cac/pkg/k8s/selector/selection"
	"github.com/rprakashg/cac/pkg/queryparser"
	"github.com/rprakashg/cac/pkg/queryparser/sqljsonb"
...
func NewAnnotationSelector(input string) (*AnnotationSelector, error) {
	...
	if err != nil {
		return nil, NewSelectorError(errors.ErrAnnotationSelectorSyntax, err)
	}
...
func (s *AnnotationSelector) Parse(ctx context.Context, name SelectorName, resolver Resolver) (string, []any, error) {
	if resolver == nil {
		return "", nil, NewSelectorError(errors.ErrAnnotationSelectorParseFailed,
	...
	if err != nil {
		return "", nil, NewSelectorError(errors.ErrAnnotationSelectorParseFailed, err)
	}
