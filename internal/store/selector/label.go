...
import (
	...
	"github.com/rprakashg/cac/pkg/k8s/selector/labels"
...
func NewLabelSelector(input string) (*LabelSelector, error) {
	...
	if err != nil {
		return nil, NewSelectorError(errors.ErrLabelSelectorSyntax, err)
	}
...
func (ls *LabelSelector) Parse(ctx context.Context, name SelectorName, resolver Resolver) (string, []any, error) {
	if resolver == nil {
		return "", nil, NewSelectorError(errors.ErrLabelSelectorParseFailed,
	...
	if err != nil {
		return "", nil, NewSelectorError(errors.ErrLabelSelectorParseFailed, err)
	}
